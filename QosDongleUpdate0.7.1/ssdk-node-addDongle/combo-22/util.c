...
static bool processLCMsg(EmberAfClusterCommand *cmd)
{
#ifdef CWP_NODE
  zigbeelink_Zcl_Receive_t zclReceiveMsg;
  zclReceiveMsg.clusterId = cmd->apsFrame->clusterId;
  zclReceiveMsg.srcAddress.address.zAddress = cmd->source;
  zclReceiveMsg.srcAddress.endpoint = cmd->apsFrame->sourceEndpoint;
  uint16_t destaddr;
  uint16_t myaddr = emberGetNodeId();
  switch(cmd->type){
  case EMBER_INCOMING_UNICAST:
	   destaddr = myaddr;
	   zclReceiveMsg.destAddress.mode = zigbee_AddrMode_Short;
	   break;
  case EMBER_INCOMING_MULTICAST:
  case EMBER_INCOMING_MULTICAST_LOOPBACK:
       destaddr = cmd->apsFrame->groupId;
       zclReceiveMsg.destAddress.mode = zigbee_AddrMode_Group;
	   break;
  case EMBER_INCOMING_BROADCAST:
	   destaddr = 0xFFFF;
	   zclReceiveMsg.destAddress.mode = zigbee_AddrMode_Broadcast;
       break;
  default:
       break;
  }

  zclReceiveMsg.destAddress.address.zAddress = destaddr;
  zclReceiveMsg.destAddress.endpoint = cmd->apsFrame->destinationEndpoint;
  zclReceiveMsg.zclFrame.payload.length = cmd->bufLen - cmd->payloadStartIndex;
  zclReceiveMsg.zclFrame.header.commandId = cmd->commandId;
  zclReceiveMsg.zclFrame.header.frameControl.raw = cmd->buffer[0];
  if(zclReceiveMsg.zclFrame.header.frameControl.detailed.manufacturerSpecific == 1)
  {
#if 0
    uint16_t manuCode = 0;
	manuCode = ((uint16_t)cmd->buffer[1]) << 8 + (uint16_t)cmd->buffer[2];
    zclReceiveMsg.zclFrame.header.manufacturerCode = manuCode;
	emberAfDebugPrintln("manufacturerCode: %d ,%d, %d",zclReceiveMsg.zclFrame.header.manufacturerCode,cmd->buffer[1],cmd->buffer[2]);
#endif
    zclReceiveMsg.zclFrame.header.manufacturerCode = 0x100B;
  }
  zclReceiveMsg.zclFrame.header.transSeqNum = cmd->seqNum;

  memcpy(&(zclReceiveMsg.zclFrame.payload.data), &cmd->buffer[cmd->payloadStartIndex], zclReceiveMsg.zclFrame.payload.length);

#if 0
  uint8_t bufFmt[90] = {0};
  uint8_t buf[40] = {0};
  uint8_t len = 0;
  buf[len++] = cmd->buffer[0];
  if(zclReceiveMsg.zclFrame.header.frameControl.detailed.manufacturerSpecific == 1)
  {
    buf[len++] = cmd->buffer[1];
    buf[len++] = cmd->buffer[2];
  }
  buf[len++] = cmd->seqNum;
  buf[len++] = cmd->commandId;
  memcpy(&buf[len],(cmd->buffer + cmd->payloadStartIndex), (cmd->bufLen - cmd->payloadStartIndex));
  len += (cmd->bufLen - cmd->payloadStartIndex);
  sprintf((char*)bufFmt,"ZclInSilabs,S=0x%04x.%d,D=0x%04x.%d,0x%04x,",
					   cmd->source,cmd->apsFrame->sourceEndpoint,destaddr,cmd->apsFrame->destinationEndpoint,cmd->apsFrame->clusterId);
  log_PrintBuf(bufFmt, buf, len);
#endif
  if(zclReceiveMsg.srcAddress.address.zAddress == myaddr)
  {
     zclReceiveMsg.destAddress.address.zAddress = 0;
     zclReceiveMsg.destAddress.mode = zigbee_AddrMode_Short;
  }

  if(zclReceiveMsg.clusterId == ZCL_OCCUPANCY_SENSING_CLUSTER_ID)
  {
	zclReceiveMsg.destAddress.address.zAddress = 0;
	zclReceiveMsg.srcAddress.address.zAddress = myaddr;
	zclReceiveMsg.destAddress.mode = zigbee_AddrMode_Short;
  }

  if(lc_Fsm_HandleIncomingZclMsg(&zclReceiveMsg,myaddr))  //consumed
    return true;

  emberAfDebugPrintln("*********************NOT CONSUMED**********************");
    return false;
#else
    return false;
#endif
...
EmberStatus emberAfSendResponse_EnableDefaultResp(void)
{
