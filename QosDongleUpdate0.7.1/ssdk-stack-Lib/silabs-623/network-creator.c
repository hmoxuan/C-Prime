...
bool getFormCentralizedNetwork()
{
  return doFormCentralizedNetwork;
}
...
{
    uint8_t channel, channelIndex = 0;
    int8_t lowestRssi;
    int32_t lowestRssiChannel = -1;   //-1 is invalid channel
    uint32_t channelMask = 0;

    for (channel = EMBER_MIN_802_15_4_CHANNEL_NUMBER; channel <= EMBER_MAX_802_15_4_CHANNEL_NUMBER; channel++)
    {
        // channel must be a primary channel
        if (READBIT(EMBER_AF_PLUGIN_NETWORK_CREATOR_CHANNEL_MASK, channel))
        {
            channelIndex = channel - EMBER_MIN_802_15_4_CHANNEL_NUMBER;
            if (channelComposites[channelIndex].maxRssiHeard != EM_AF_PLUGIN_NETWORK_CREATOR_CHANNEL_COMPOSITE_INVALID_RSSI)
            {
                if (lowestRssiChannel < 0)
                {
                    lowestRssi = channelComposites[channelIndex].maxRssiHeard;
                    lowestRssiChannel = channel;
                }
                else
                {
                    if (lowestRssi > channelComposites[channelIndex].maxRssiHeard)
                    {
                        lowestRssi = channelComposites[channelIndex].maxRssiHeard;
                        lowestRssiChannel = channel;
                    }
    ...
    if (lowestRssiChannel < 0)
    {
        lowestRssiChannel = EMBER_MIN_802_15_4_CHANNEL_NUMBER;
    }
    SETBIT(channelMask, (uint8_t)lowestRssiChannel);
    return channelMask;
}
  ...
  if (results->status != EMBER_SUCCESS) {
    ...
    if (scanType == EMBER_ENERGY_SCAN) {
      ...
      if (results->status != EMBER_SUCCESS) {
        ...
        if (!maskIsSecondary()) {
          currentChannelMask = GetLowestRssiChannelMask();
