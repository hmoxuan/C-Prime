...
#include "app/framework/plugin/zll-commissioning/zll-commissioning.h" //emberAfZllSetInitialSecurityState()
...
// #define PLUGIN_DEBUG
#if defined(EMBER_AF_PRINT_ENABLE) && defined(EMBER_AF_PRINT_NETWORK_STEERING)
#define LOCAL_DEBUG
#endif
...
void printXpanBigEndianEui64(const EmberEUI64 eui64)
{
  log_Printfbasic("xpan:(%c)%X%X%X%X%X%X%X%X",
               '>',
               eui64[7],
               eui64[6],
               eui64[5],
  ...
  #define debugPrintln      log_Printfbasic
  #define debugPrint        log_Printfbasic
  #define emberAfPrintBigEndianEui64(x)    printXpanBigEndianEui64(x)
  #define debugExec(x) do { x; } while(0)
#else
  #define debugPrintln(...)
  #define debugPrint(...)
...
static bool s_bStopSteering = false;
//============================================================================
                                 ...
                                 int8_t rssi)
{
  ...
  debugPrint("%p nwk found ch: %d, panID 0x%2X, rssi:%d",
             PLUGIN_NAME,
...
static void cleanupAndStop(EmberStatus status)
{
  ...
  if ((status != EMBER_SUCCESS) && (s_bStopSteering != true))
  {
    emberEventControlSetDelayMS(commissioningLedEventControl, 5000);
  }
}

void network_steering_cleanupAndStop(EmberStatus status)
{
  emberAfCorePrintln("%p Stop.  Cleaning up.", PLUGIN_NAME);
  emberAfPluginNetworkSteeringCompleteCallback(status,
                                               emAfPluginNetworkSteeringTotalBeacons,
...
static EmberStatus stateMachineRun(void)
{
  ...
  emberAfCorePrintln("%p State: %p(%d)",
                     PLUGIN_NAME,
